(function() {
    ...
    function loadState() {
        ...
        if (savedActive === 'true' && savedStartTime && savedStartTime !== 'undefined') {
    ...
    function blockAllInteractions() {
        ...
        blockedHandlers = [blockEvent, blockDevTools, blockRightClick, blockContextMenu];
        ...
        ['mousedown', 'mouseup'].forEach(event => {
            document.addEventListener(event, blockRightClick, { capture: true, passive: false });
        });
        ...
        document.addEventListener('keyup', blockDevTools, { capture: true, passive: false });
    ...
    function restoreAllInteractions() {
        ...
        if (blockedHandlers.length > 0) {
            ['scroll', 'wheel', 'touchmove', 'click', 'mousemove'].forEach(event => {
                document.removeEventListener(event, blockedHandlers[0], { capture: true });
            });
            
            ['mousedown', 'mouseup'].forEach(event => {
                document.removeEventListener(event, blockedHandlers[2], { capture: true });
            });
            
            document.removeEventListener('contextmenu', blockedHandlers[3], { capture: true });
            document.removeEventListener('keydown', blockedHandlers[1], { capture: true });
            document.removeEventListener('keyup', blockedHandlers[1], { capture: true });
